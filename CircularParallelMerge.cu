...
__global__ void merge_circular_kernel(int *A, int m, int *B, int n, int *C, int tile_size) {
    ...
    while (counter < total_iterations) {
        ...
        for (int i = 0; i < A_S_consumed; i+=blockDim.x) {
            if (i + threadIdx.x < A_length - A_consumed && (i + threadIdx.x) < A_S_consumed) {
                // Index has changed: A_S_start is just after the last item that was taken last iteration
                // tile_size - A_S_consumed is what's left over from last iteration
                // i + threadIdx.x is still the offset to load
